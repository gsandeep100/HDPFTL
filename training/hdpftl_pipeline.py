...
def hdpftl_pipeline(X_train, y_train, X_test, y_test, base_model_fn,
    ...
    for cid in range(num_clients):
        ...
        trained_model = train_device_model(model, X_train[idx], y_train[idx], device)
        local_models.append(trained_model)

    print("\n[3] Aggregating fleet models...")
    global_model = aggregate_models(local_models, base_model_fn, device)

    print("\n[4] Evaluating global model...")
    evaluate_global_model(global_model, X_test, y_test, device=device)
    evaluate_per_client(global_model, X_train, y_train, client_partitions, device=device)

    print("\n[5] Personalizing each client...")
    personalized_models = personalize_clients(global_model, X_train, y_train, client_partitions, device=device)
